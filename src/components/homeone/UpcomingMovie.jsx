...
const UpcomingMovie = () => {
  ...
  useEffect( async () => {
    await axios.get("http://localhost:3000/movie/").then((response) => {
      ...
      <div className="row justify-content-center">
        <div className="col-lg-8">
          <div className="tr-movie-menu2-active text-center">
            <button className="active" data-filter="*" onClick={()=> setItems(post)}>TV Shows</button>
      ...
      {/* <div className="row tr-movie-active">
        {
               post.map((elem)=>{
                ...
                const {id,poster,title,year,quality,video} = elem;
                const imgsrc = String(poster).split('/')[4];
                console.log('asdfasdfasdfasdf',imgsrc);
                const image = './poster/' + imgsrc;
